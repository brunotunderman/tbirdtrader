...
export default function LiveChart() {
  ...
  const fetchCandlesAndPrediction = useCallback(async (tf: Timeframe) => {
    ...
    try {
      ...
      // --- Debug candle coverage ---
      if (formatted.length > 0) {
        const first = formatted[0].time;
        const last = formatted[formatted.length - 1].time;
        console.log(
          `[candles ${data.meta?.interval}] count=${formatted.length} first=${new Date(
            first * 1000
          ).toISOString()} last=${new Date(last * 1000).toISOString()}`
        );
      }
