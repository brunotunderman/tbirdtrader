...
export default function TradePage() {
  ...
  const { interval, limit } = getParamsForTimeframe(timeframe);

  const res = await fetch(
    `http://localhost:8000/market/candles?symbol=BTC-EUR&interval=${interval}&limit=${limit}`
  );
...
async function load() {
  ...
  const res = await fetch(
    `http://localhost:8000/market/candles?symbol=${symbol}&interval=${interval}&limit=${limit}`
  );
...
useEffect(() => {
  ...
  function getParamsForTimeframe(tf) {
    switch (tf) {
    ...
    //  case "5Y": return { interval: "1d", limit: 1825 };
    //  case "ALL": return { interval: "1d", limit: 5000 };
      default: return { interval: "1h", limit: 200 };
   }
 }
  ...
  {["1D", "1W", "1M", "1Y"].map(tf => (
    <button
      ...
      className={`px-3 py-1 rounded border ${
        timeframe === tf ? "bg-blue-600 text-white" : "bg-white text-gray-700"
      }`}
    >
      {tf}
    </button>
...
</section><section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
    ...
    {account && account.trade_history.length > 0 && (
      <div className="space-y-3">
        {account.trade_history.map((trade, idx) => {
          ...
          const badgeColor = isBuy ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
          const badgeLabel = isBuy ? "Bought" : "Sold"

          return (
            <div
              key={idx}
              className="p-4 bg-gray-50 rounded-lg border hover:shadow transition-all"
            >
              ...
              <div className="text-sm text-gray-700 space-y-1">
                <div>
                  <span className="font-medium">Amount:</span> {trade.amount}
                </div>
                <div>
                  <span className="font-medium">Price:</span> ${trade.price}
                </div>
              </div>
            </div>
          )
